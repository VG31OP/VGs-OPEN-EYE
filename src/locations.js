...
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  if (apiKey) {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        types = [first.class || 'place', first.type || 'point_of_interest'];
        if (first.boundingbox && first.boundingbox.length === 4) {
          viewport = {
            southwest: { lat: Number.parseFloat(first.boundingbox[0]), lng: Number.parseFloat(first.boundingbox[2]) },
            northeast: { lat: Number.parseFloat(first.boundingbox[1]), lng: Number.parseFloat(first.boundingbox[3]) },
          };
        }
      }
    } catch (err) {
      console.warn('[Geocode] Nominatim geocode fallback failed:', err);
    }
  }
  ...
  const hasResult = Number.isFinite(lat) && Number.isFinite(lng);
  const recovered = await placesNearViewRecovery(viewer, query, hasResult ? { lat, lon: lng } : null);
  if (recovered) {
    lat = recovered.lat;
    lng = recovered.lon;
