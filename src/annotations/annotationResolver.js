...
async function geocodePlace(query, biasRect, signal) {
  ...
  if (apiKey) {
    ...
    try {
      ...
      const first = data[0];
      const primaryName = first.display_name ? first.display_name.split(',')[0].trim() : query;
      const place = {
        ...
        viewport: first.boundingbox && first.boundingbox.length === 4 ? {
          low: { latitude: Number.parseFloat(first.boundingbox[0]), longitude: Number.parseFloat(first.boundingbox[2]) },
          high: { latitude: Number.parseFloat(first.boundingbox[1]), longitude: Number.parseFloat(first.boundingbox[3]) },
        } : null,
      };
      cacheWrite(geocodeCache, cacheKey, place);
      return place;
