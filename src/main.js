...
/**
 * VG's OPEN EYE — Main Entry Point
...
async function init() {
  ...
  try {
    ...
    // Set Google Maps API key for 3D Tiles if provided
    ...
    // Keep Cesium globe visible by default for keyless / open map stacks.
    viewer.scene.globe.show = true;
    viewer.scene.globe.baseColor = Cesium.Color.fromCssColorString('#020914');
    viewer.scene.globe.showGroundAtmosphere = true;

    // Subtle cyan/blue atmospheric rim glow
    ...
    if (googleApiKey) {
      ...
      try {
        // Load Google Photorealistic 3D Tiles when API key is provided
    ...
    window.__vgOpenEye.voiceCommands = initGevVoiceCommands({ viewer, styleManager, dataManager, sceneDirector, annotations });
    window.__godsEyeView = window.__vgOpenEye;

  } catch (error) {
    console.error("VG's OPEN EYE initialization failed:", error);
