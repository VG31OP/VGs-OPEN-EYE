...
const VOICE_TIER_STORAGE_KEY = 'vgOpenEye.voiceCost.tier';
const VOICE_TIER_LEGACY_STORAGE_KEY = 'godsEyeView.voiceCost.tier';
const VOICE_LIMITS_STORAGE_KEY = 'vgOpenEye.voiceCost.limits';
const VOICE_LIMITS_LEGACY_STORAGE_KEY = 'godsEyeView.voiceCost.limits';

function getMigratedVoiceItem(store, newKey, legacyKey) {
  if (!store) return null;
  const val = store.getItem(newKey);
  if (val !== null && val !== undefined) return val;
  const legacyVal = store.getItem(legacyKey);
  if (legacyVal !== null && legacyVal !== undefined) {
    try { store.setItem(newKey, legacyVal); } catch {}
    return legacyVal;
  }
  return null;
}
...
export function readStoredVoiceTier(storage) {
  try {
    const store = voiceStorage(storage);
    const raw = getMigratedVoiceItem(store, VOICE_TIER_STORAGE_KEY, VOICE_TIER_LEGACY_STORAGE_KEY);
...
export function readStoredVoiceLimits(storage) {
  try {
    const store = voiceStorage(storage);
    const raw = getMigratedVoiceItem(store, VOICE_LIMITS_STORAGE_KEY, VOICE_LIMITS_LEGACY_STORAGE_KEY);
    ...
    const contextEvent = {
      ...
      item: {
        ...
        content: [
          {
            type: 'input_text',
            text: "Current VG's OPEN EYE viewport screenshot. Read any clearly visible street, building, and place labels in the image and combine them with the structured nearbyPlaces, streetLabels, and scene context. Do not invent labels that are not legible.",
          },
          {
            type: 'input_image',
            image_url: imageUrl,
  ...
  syncCostUi() {
    ...
    if (this.ui?.tierButton) {
      ...
      this.ui.tierButton.title = this.isActive() && state.modelId !== pendingId
        ...
        : `Voice model: ${pendingId} — click to switch to ${isMini ? 'standard' : 'mini'
...
async function captureViewportImage() {
  const viewer = (window.__vgOpenEye || window.__godsEyeView)?.viewer;
