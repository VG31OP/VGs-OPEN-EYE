...
test('an unset or hand-edited tier reads back as standard', () => {
  ...
  assert.equal(
    readStoredVoiceTier(fakeVoiceStorage({ 'vgOpenEye.voiceCost.tier': 'gpt-4o' })),
    'standard'
  );
  assert.equal(
  ...
  assert.equal(
    readStoredVoiceTier(fakeVoiceStorage({ 'vgOpenEye.voiceCost.tier': '__proto__' })),
    'standard'
  );
});
...
test('writing a bogus tier persists the safe fallback, not the bogus value', () => {
  ...
  assert.equal(storage.dump()['vgOpenEye.voiceCost.tier'], 'standard');
});
...
test('corrupt stored limits fall back to defaults rather than disarming the cap', () => {
  ...
  const limits = readStoredVoiceLimits(
    fakeVoiceStorage({ 'vgOpenEye.voiceCost.limits': '{oops' })
  );
...
test('partially stored limits keep the default for the missing threshold', () => {
  const limits = readStoredVoiceLimits(
    fakeVoiceStorage({ 'vgOpenEye.voiceCost.limits': '{"warnUsd":0.5}' })
  );
  assert.deepEqual(limits, { warnUsd: 0.5, capUsd: 5 });
});

test('legacy godsEyeView stored limits migrate seamlessly', () => {
  const limits = readStoredVoiceLimits(
...
test('a disabled threshold survives the storage round-trip', () => {
  ...
  const raw = storage.dump()['vgOpenEye.voiceCost.limits'];
