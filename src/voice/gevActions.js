...
async function getBasemapContext(viewer, viewTarget = null) {
  ...
  if (!target) {
    ...
    return {
      ...
      hasGoogle3DTiles: Boolean((window.__vgOpenEye || window.__godsEyeView)?.tileset),
      viewScale,
    ...
    try {
      if (apiKey) {
        ...
        const place = {
          ...
          types: [data.class || 'place', data.type || 'locality'],
          labels: [data.display_name],
          streetLabels: addr.road ? [addr.road] : [],
        };
        reverseGeocodeCache.set(key, place);
        return place;
      }
