...
/**
 * Central UI orchestrator for the VG's OPEN EYE application.
 *
 * Responsibilities:
...
export class StyleManager {
  ...
  _maybeNotifyLayoutReset() {
    try {
      const marker = `vgOpenEye.${PANEL_POSITION_STORAGE_VERSION}.layoutResetNotified`;
      const legacyMarker = `godsEyeView.${PANEL_POSITION_STORAGE_VERSION}.layoutResetNotified`;
      if (localStorage.getItem(marker) || localStorage.getItem(legacyMarker)) return;
      localStorage.setItem(marker, '1');
      const hadOldPositions = Object.keys(localStorage)
        .some((key) => key.startsWith('vgOpenEye.v6.panelPos.') || key.startsWith('godsEyeView.v6.panelPos.'));
      if (hadOldPositions) {
  ...
  _panelStorageKey(panelId) {
    return `vgOpenEye.${PANEL_POSITION_STORAGE_VERSION}.panelPos.${panelId}`;
  }

  _legacyPanelStorageKey(panelId) {
  ...
  _panelCollapseStorageKey(panelId) {
    return `vgOpenEye.${PANEL_LAYOUT_STORAGE_VERSION}.panelCollapsed.${panelId}`;
  }

  _legacyPanelCollapseStorageKey(panelId) {
    ...
    if (allowStored) {
      try {
        ...
        if (stored === null || stored === undefined) {
          const legacyStored = localStorage.getItem(this._legacyPanelCollapseStorageKey(panelId));
          if (legacyStored !== null && legacyStored !== undefined) {
            stored = legacyStored;
            try { localStorage.setItem(this._panelCollapseStorageKey(panelId), legacyStored); } catch {}
          }
        }
