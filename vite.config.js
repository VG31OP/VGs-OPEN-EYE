/**
 * Vite configuration for VG's OPEN EYE — a cinematic geospatial app.
 *
...
const RADIO_USER_AGENT = 'VG-OPEN-EYE/1.0 (Radio Browser directory client)';
...
function celestrakProxy() {
  ...
  async function fetchUpstream(group) {
    ...
    const res = await fetch(url.toString(), {
      ...
      headers: { 'User-Agent': 'vg-open-eye-celestrak-proxy/1.0 (+https://github.com/VG31OP/VG-s-OPEN-EYE)' },
    });
    ...
    try {
      const upstream = await fetch(endpoint, {
        ...
        headers: {
          ...
          'User-Agent': 'vg-open-eye-overpass-proxy/1.0',
        },
        body,
        signal: controller.signal,
      });
        ...
        try {
          ...
          try {
            const upstreamRes = await fetch(upstream, {
              signal: controller.signal,
              headers: { 'User-Agent': 'vg-open-eye/dev (local)' },
            });
    ...
    try {
      const upstream = await fetch(
        `https://api.adsb.lol/v2/lat/${roundedLat}/lon/${roundedLon}/dist/${ADSBLOL_POINT_RADIUS_NM}`,
        {
          headers: {
            Accept: 'application/json',
            'User-Agent': 'vg-open-eye-adsblol-regional-fallback/1.0',
          },
          signal: controller.signal,
        },
      );
        ...
        try {
          ...
          try {
            upstream = await fetch(upstreamUrl.toString(), {
              ...
              headers: {
                Accept: 'application/json',
                'User-Agent': 'vg-open-eye-gbfs-proxy/1.0',
              },
              signal: controller.signal,
            });
          } finally {
...
export async function fetchCctvImageFromUpstream(url, {
  ...
  try {
    const upstream = await fetchImpl(url, {
      headers: { 'User-Agent': 'vg-open-eye-cctv-proxy/1.0' },
      signal: controller.signal,
    });
      ...
      server.middlewares.use('/api/cctv', async (req, res) => {
        try {
          ...
          if (url.pathname.startsWith('/media/')) {
            ...
            try {
              const upstreamHeaders = { 'User-Agent': 'vg-open-eye-cctv-proxy/1.0' };
      ...
      server.middlewares.use('/api/adsblol/mil', async (req, res) => {
        try {
          ...
          const upstream = await fetch('https://api.adsb.lol/v2/mil', {
            headers: { 'User-Agent': 'vg-open-eye-adsblol-proxy/1.0' },
          });
          const body = await upstream.text();
          if (upstream.ok) {
            _cache = body;
      ...
      try {
        ...
        const response = await fetch('https://api.openai.com/v1/responses', {
          ...
          body: JSON.stringify({
            ...
            instructions: [
              "Write one concise intelligence-HUD summary for VG's OPEN EYE.",
    ...
    middlewares.use('/api/realtime/token', async (req, res) => {
      ...
      const sessionConfig = {
        session: {
          ...
          instructions: [
            "You are GEV Voice Control, a concise voice controller for a Cesium geospatial app called VG's OPEN EYE.",
...
const GEV_REALTIME_TOOLS = [
  {
    ...
    description: "Fly the VG's OPEN EYE camera to a known city, geocoded country/region/city/landmark, or explicit WGS84 coordinate. Countries/cities frame the whole place; landmarks/buildings use close framing.",
    parameters: {
      type: 'object',
  ...
  },
  {
    ...
    description: "Enable or disable one registered VG's OPEN EYE data layer.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  },
  {
    ...
    description: "Set the active VG's OPEN EYE visual filter/style.",
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
...
function fetchRegionalPlace(point) {
  const task = _nominatimQueue.then(async () => {
    ...
    const payload = await fetchRegionalJson(`https://nominatim.openstreetmap.org/reverse?${params}`, {
      headers: {
        'User-Agent': 'VG-OPEN-EYE/0.1 (+https://github.com/VG31OP/VG-s-OPEN-EYE)',
        Referer: 'https://github.com/VG31OP/VG-s-OPEN-EYE',
      },
    });
...
async function fetchRegionalNews(place) {
  ...
  try {
    const xml = await fetchRegionalText(`https://news.google.com/rss/search?${rssParams}`, {
      headers: { 'User-Agent': 'VG-OPEN-EYE/0.1' },
      timeoutMs: 12_000,
    });
